...
export function paises(divPublicacionContainer) {
  return state.forEach((country) => {
    const selectCountry = divPublicacionContainer.querySelector('#selectCountry');
    // Select países
    selectCountry.innerHTML += `<option value='${country}'>${country}</option>`;
  });
}

// const arr = ['pais', 'per', 'bol'];
